...
export function ReminderTimePicker({ label, value, error, onChange }: ReminderTimePickerProps) {
  ...
  return (
    <View className="gap-2">
      ...
      {Platform.OS === 'android' ? (
        <Button variant="ghost" className="justify-between" onPress={openAndroidPicker}>
        ...
        <View
          className={`rounded-2xl border bg-white px-2 py-1 dark:bg-charcoal-900 ${
            error ? 'border-red-500 dark:border-red-400' : 'border-sage-200 dark:border-charcoal-700'
          }`}
        >
          <DateTimePicker
            value={reminderTimeToDate(value)}
